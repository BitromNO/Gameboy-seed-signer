...
static void print_hex16(uint16_t value) {
  ...
  printf("%c%c%c%c", digits[(value >> 12) & 0x0Fu], digits[(value >> 8) & 0x0Fu], digits[(value >> 4) & 0x0Fu], digits[value & 0x0Fu]);
}
...
static void draw_menu(void) {
  ...
  gotoxy(1, 15); printf("UP/DN A:OPEN");
  gotoxy(1, 17); printf("B:BACK");
}
static void draw_active_screen(void) {
  ...
  if (active_screen == SCREEN_READY) {
    ...
    if (dice_count >= DICE_TARGET) {
      gotoxy(3, 8); printf("ENTROPY: "); print_hex16(((uint16_t)dice_entropy[0] << 8) | dice_entropy[1]);
...
static void show_boot_logo(void) {
  ...
  for (tick = 0; tick != 48u; tick++) { set_bkg_data(128, 1, coin_frames[tick & 0x03u]); set_bkg_tiles(9, 7, 1, 1, coin_tile); wait_vbl_done(); delay(35u); }
}
void main(void) {
  ...
  while (1) {
    ...
    if (!(previous_keys & J_A) && (keys & J_A) && active_screen == SCREEN_DICE && dice_count != DICE_TARGET) { play_confirm_sound(); dice_rolls[dice_count] = (uint8_t)('0' + dice_value); dice_audit = (uint16_t)(((dice_audit << 5) | (dice_audit >> 11)) ^ dice_value); dice_count++; if (dice_count == DICE_TARGET) sha256_dice_rolls(); draw_active_screen(); }
