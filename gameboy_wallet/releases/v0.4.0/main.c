...
typedef enum Screen { SCREEN_MENU, SCREEN_READY, SCREEN_ADDRESS, SCREEN_DICE, SCREEN_WORDS, SCREEN_INBOX, SCREEN_REVIEW, SCREEN_TOOLS, SCREEN_SETTINGS, SCREEN_HODL } Screen;
...
static const char *const menu_items[MENU_ITEM_COUNT] = { "READY TO SIGN", "ADDRESS EXPLORER", "DICE AUDIT", "SEED WORDS", "FILE MANAGEMENT", "REVIEW TX", "ADVANCED TOOLS", "SETTINGS", "HODL MODE" };
static const Screen menu_screens[MENU_ITEM_COUNT] = { SCREEN_READY, SCREEN_ADDRESS, SCREEN_DICE, SCREEN_WORDS, SCREEN_INBOX, SCREEN_REVIEW, SCREEN_TOOLS, SCREEN_SETTINGS, SCREEN_HODL };
...
static void draw_active_screen(void) {
  ...
  if (active_screen == SCREEN_READY) {
    ...
    if (dice_count >= DICE_TARGET) {
      ...
      print_centered(14, "A:NEXT SELECT:WORDS"); print_centered(16, "B:WIPE AND EXIT");
    } else {
...
void main(void) {
  ...
  SHOW_BKG; sound_init(); load_seed_session(); show_boot_logo(); draw_menu();
  while (1) {
    keys = joypad();
    ...
    if (!(previous_keys & J_SELECT) && (keys & J_SELECT) && active_screen == SCREEN_DICE && dice_count == DICE_TARGET) { active_screen = SCREEN_WORDS; draw_active_screen(); }
