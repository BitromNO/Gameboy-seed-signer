...
typedef enum Screen { SCREEN_MENU, SCREEN_READY, SCREEN_ADDRESS, SCREEN_DICE, SCREEN_INBOX, SCREEN_REVIEW, SCREEN_TOOLS, SCREEN_SETTINGS, SCREEN_HODL } Screen;
...
static const char *const menu_items[MENU_ITEM_COUNT] = { "READY TO SIGN", "ADDRESS EXPLORER", "DICE AUDIT", "FILE MANAGEMENT", "REVIEW TX", "ADVANCED TOOLS", "SETTINGS", "HODL MODE" };
static const Screen menu_screens[MENU_ITEM_COUNT] = { SCREEN_READY, SCREEN_ADDRESS, SCREEN_DICE, SCREEN_INBOX, SCREEN_REVIEW, SCREEN_TOOLS, SCREEN_SETTINGS, SCREEN_HODL };
static const unsigned char coin_frames[][8] = {
  { 0x3C, 0x7E, 0x5A, 0x7E, 0x5A, 0x7E, 0x3C, 0x00 },
  { 0x18, 0x7E, 0x3C, 0x7E, 0x3C, 0x7E, 0x18, 0x00 },
  { 0x3C, 0x7E, 0x6A, 0x7E, 0x6A, 0x7E, 0x3C, 0x00 },
  { 0x18, 0x7E, 0x5A, 0x7E, 0x5A, 0x7E, 0x18, 0x00 }
};
static const unsigned char coin_tile[] = { 128 };
static const unsigned char blank_line[20] = { 0 };
static const uint16_t hold_melody[] = { 1546u, 1735u, 2063u, 1735u, 1546u, NOTE_REST, 1546u, 1735u, 2063u, 2314u, 2063u, 1735u, 1546u, NOTE_REST, 1379u, 1546u, 1735u, 2063u };
static uint8_t selected_item, music_enabled = 1u, melody_step, melody_clock;
static uint8_t dice_value = 1u, dice_count;
...
static void sound_init(void) { NR52_REG = 0x80u; NR50_REG = 0x77u; NR51_REG = 0x11u; NR10_REG = 0x00u; NR11_REG = 0x80u; NR12_REG = 0x72u; }
static void play_note(uint16_t frequency) {
  if (!music_enabled) return;
  if (frequency == NOTE_REST) { NR12_REG = 0x00u; return; }
  NR12_REG = 0x72u; NR13_REG = (uint8_t)frequency; NR14_REG = (uint8_t)(0x80u | (frequency >> 8));
}
static void update_music(void) {
  if (++melody_clock != 18u) return;
  melody_clock = 0; play_note(hold_melody[melody_step]);
  if (++melody_step == (sizeof(hold_melody) / sizeof(hold_melody[0]))) melody_step = 0;
}
static uint32_t rotr(uint32_t value, uint8_t count) { return (value >> count) | (value << (32u - count)); }
static void sha256_dice_rolls(void) {
  ...
  uint8_t index, round, block_index;

  for (index = 0; index != 99u; index++) block[index] = dice_rolls[index];
  block[99] = 0x80u;
  for (index = 100u; index != 128u; index++) block[index] = 0u;
  block[126] = 0x03u;
  block[127] = 0x18u;
  for (index = 0; index != 8u; index++) state[index] = initial_state[index];
  for (block_index = 0; block_index != 2u; block_index++) {
  ...
  for (index = 0; index != 8u; index++) { dice_entropy[index * 4u] = (uint8_t)(state[index] >> 24); dice_entropy[index * 4u + 1u] = (uint8_t)(state[index] >> 16); dice_entropy[index * 4u + 2u] = (uint8_t)(state[index] >> 8); dice_entropy[index * 4u + 3u] = (uint8_t)state[index]; }
}
...
static void draw_menu(void) {
  uint8_t index;
  draw_frame("POCKET BTC"); gotoxy(1, 3); printf("MAINNET BLOCK 840K");
  for (index = 0; index != MENU_ITEM_COUNT; index++) {
    uint8_t column; const char *label = menu_items[index];
    gotoxy(1, (uint8_t)(4u + index)); printf("%c ", index == selected_item ? '>' : ' ');
    for (column = 0; label[column] && column != 16u; column++) printf("%c", label[column]);
    while (column++ != 16u) printf(" ");
  }
  gotoxy(1, 15); printf("%u/%u UP/DN A:OPEN", (uint8_t)(selected_item + 1u), MENU_ITEM_COUNT);
  gotoxy(1, 17); printf("START:MUSIC %s", music_enabled ? "ON " : "OFF");
}
static void draw_active_screen(void) {
  ...
  if (active_screen == SCREEN_READY) {
    ...
    gotoxy(3, 4); printf("ROLL: %u", dice_value);
    gotoxy(3, 6); printf("COUNT: %u/%u", dice_count, DICE_TARGET);
    if (dice_count >= DICE_TARGET) {
      gotoxy(3, 8); printf("ENTROPY: %x", dice_entropy[0] << 8 | dice_entropy[1]);
      print_centered(11, "256-BIT SEED READY"); print_centered(13, "SHA256 OF 99 ROLLS"); print_centered(16, "VOLATILE B:DISCARD");
    } else {
      gotoxy(3, 8); printf("AUDIT: %x", dice_audit);
    ...
    if (inbox_is_valid()) { print_centered(4, "PACKAGE VERIFIED"); print_centered(6, "SRAM INBOX READY"); print_centered(8, "PAYLOAD BYTES:"); gotoxy(8, 10); printf("%u", (uint16_t)SRAM_BASE[6] | ((uint16_t)SRAM_BASE[7] << 8)); }
  ...
  else { draw_frame("SETTINGS"); print_centered(4, "NETWORK: MAINNET"); print_centered(6, "SIGNER: DISABLED"); print_centered(8, music_enabled ? "SOUND: ON" : "SOUND: OFF"); print_centered(10, "DEMO BUILD 0.1"); print_centered(16, "START:MUSIC B:BACK"); return; }
...
static void show_boot_logo(void) {
  ...
  for (tick = 0; tick != 48u; tick++) { set_bkg_data(128, 1, coin_frames[tick & 0x03u]); set_bkg_tiles(9, 7, 1, 1, coin_tile); if ((tick & 0x03u) == 0u) play_note(hold_melody[(tick >> 2) % 6u]); wait_vbl_done(); delay(35u); }
}
void main(void) {
  ...
  while (1) {
    ...
    if (!(previous_keys & J_A) && (keys & J_A) && active_screen == SCREEN_MENU) { active_screen = menu_screens[selected_item]; draw_active_screen(); }
    if (!(previous_keys & J_A) && (keys & J_A) && active_screen == SCREEN_DICE && dice_count != DICE_TARGET) { dice_rolls[dice_count] = (uint8_t)('0' + dice_value); dice_audit = (uint16_t)(((dice_audit << 5) | (dice_audit >> 11)) ^ dice_value); dice_count++; if (dice_count == DICE_TARGET) sha256_dice_rolls(); draw_active_screen(); }
    if (!(previous_keys & J_B) && (keys & J_B) && active_screen != SCREEN_MENU) { active_screen = SCREEN_MENU; draw_menu(); }
    if (!(previous_keys & J_START) && (keys & J_START)) { music_enabled = !music_enabled; if (!music_enabled) NR12_REG = 0x00u; draw_active_screen(); }
    previous_keys = keys; update_music(); wait_vbl_done();
  }
}