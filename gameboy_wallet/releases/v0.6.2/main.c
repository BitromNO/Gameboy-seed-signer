...
static const uint16_t coin_frames[][16] = {
  { 0x03C0u, 0x0FF0u, 0x1FF8u, 0x3FFCu, 0x7FFEu, 0x6DB6u, 0x6FB6u, 0x6DB6u, 0x6FB6u, 0x6DB6u, 0x6FB6u, 0x7FFEu, 0x3FFCu, 0x1FF8u, 0x0FF0u, 0x03C0u },
  ...
  { 0x03C0u, 0x0FF0u, 0x1FF8u, 0x3FFCu, 0x7FFEu, 0x6B76u, 0x6F76u, 0x6B76u, 0x6F76u, 0x6B76u, 0x6F76u, 0x7FFEu, 0x3FFCu, 0x1FF8u, 0x0FF0u, 0x03C0u },
...
static const uint16_t coin_symbols[][16] = {
  { 0x0000u, 0x0000u, 0x0000u, 0x0600u, 0x07C0u, 0x0660u, 0x0660u, 0x07C0u, 0x0660u, 0x0660u, 0x07C0u, 0x0600u, 0x0000u, 0x0000u, 0x0000u, 0x0000u },
  ...
  { 0x0000u, 0x0000u, 0x0000u, 0x0600u, 0x07C0u, 0x0660u, 0x07C0u, 0x0660u, 0x07C0u, 0x0660u, 0x07C0u, 0x0600u, 0x0000u, 0x0000u, 0x0000u, 0x0000u },
  { 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u }
};
static const unsigned char coin_tiles[] = { 128, 129, 130, 131 };
static const unsigned char big_coin_tiles[] = { 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147 };
static const unsigned char blank_line[20] = { 0 };
static unsigned char coin_vram[64];
static unsigned char big_coin_vram[256];
static unsigned char repo_qr_vram[1296];
static unsigned char repo_qr_tiles[81];
...
static void draw_coin_frame(uint8_t frame, uint8_t x, uint8_t y) {
  uint8_t row;
  uint16_t bitmap, symbol;

  for (row = 0; row != 16u; row++) {
    bitmap = coin_frames[frame][row];
    symbol = coin_symbols[frame][row];
    coin_vram[(row < 8u ? 0u : 2u) * 16u + (row & 7u) * 2u] = (uint8_t)(bitmap >> 8);
    coin_vram[(row < 8u ? 0u : 2u) * 16u + (row & 7u) * 2u + 1u] = (uint8_t)(symbol >> 8);
    coin_vram[(row < 8u ? 1u : 3u) * 16u + (row & 7u) * 2u] = (uint8_t)bitmap;
    coin_vram[(row < 8u ? 1u : 3u) * 16u + (row & 7u) * 2u + 1u] = (uint8_t)symbol;
  }
  set_bkg_data(128, 4, coin_vram);
  set_bkg_tiles(x, y, 2, 2, coin_tiles);
}
static void draw_big_coin_frame(uint8_t frame) {
  uint8_t source_row, source_column, tile_row, tile_column, pixel_row, pixel_column;
  uint16_t bitmap, symbol;

  for (tile_row = 0; tile_row != 4u; tile_row++) for (tile_column = 0; tile_column != 4u; tile_column++) for (pixel_row = 0; pixel_row != 8u; pixel_row++) {
    uint8_t plane_zero = 0u, plane_one = 0u;
    source_row = (uint8_t)((tile_row * 8u + pixel_row) >> 1);
    bitmap = coin_frames[frame][source_row]; symbol = coin_symbols[frame][source_row];
    for (pixel_column = 0; pixel_column != 8u; pixel_column++) {
      source_column = (uint8_t)((tile_column * 8u + pixel_column) >> 1);
      if (bitmap & ((uint16_t)0x8000u >> source_column)) plane_zero |= (uint8_t)(0x80u >> pixel_column);
      if (symbol & ((uint16_t)0x8000u >> source_column)) plane_one |= (uint8_t)(0x80u >> pixel_column);
    }
    big_coin_vram[((tile_row * 4u + tile_column) * 16u) + pixel_row * 2u] = plane_zero;
    big_coin_vram[((tile_row * 4u + tile_column) * 16u) + pixel_row * 2u + 1u] = plane_one;
  }
  set_bkg_data(132, 16, big_coin_vram);
  set_bkg_tiles(8, 5, 4, 4, big_coin_tiles);
}
static void draw_repo_qr(void) {
  uint8_t tile_row, tile_column, pixel_row, pixel_column;

  for (tile_row = 0; tile_row != 9u; tile_row++) for (tile_column = 0; tile_column != 9u; tile_column++) for (pixel_row = 0; pixel_row != 8u; pixel_row++) {
    uint8_t planes = 0u;
    for (pixel_column = 0; pixel_column != 8u; pixel_column++) {
      uint8_t module_x = (uint8_t)((tile_column * 8u + pixel_column) >> 1);
      uint8_t module_y = (uint8_t)((tile_row * 8u + pixel_row) >> 1);
      if (module_x < repo_qr_size && module_y < repo_qr_size && repo_qr_matrix[(uint16_t)module_y * repo_qr_size + module_x]) planes |= (uint8_t)(0x80u >> pixel_column);
    }
    repo_qr_vram[((tile_row * 9u + tile_column) * 16u) + pixel_row * 2u] = planes;
    repo_qr_vram[((tile_row * 9u + tile_column) * 16u) + pixel_row * 2u + 1u] = planes;
    repo_qr_tiles[tile_row * 9u + tile_column] = (uint8_t)(148u + tile_row * 9u + tile_column);
  }
  set_bkg_data(148, 81, repo_qr_vram);
  set_bkg_tiles(5, 3, 9, 9, repo_qr_tiles);
}
...
static void update_dice_progress(void) {
  gotoxy(10, 6); printf("  ");
  gotoxy(10, 6); print_u8(dice_count);
  gotoxy(10, 8); print_hex16(dice_audit);
}
static void draw_active_screen(void) {
  ...
  else if (active_screen == SCREEN_HODL) { clear_screen(); print_centered(0, "HODL HODL"); draw_repo_qr(); print_centered(13, "SCAN FOR REPO"); print_centered(15, "GITHUB SOURCE"); print_centered(17, "B:BACK"); return; }
...
static void show_boot_logo(void) {
  ...
  for (tick = 0; tick != 20u; tick++) { draw_big_coin_frame((uint8_t)((tick / 5u) & 0x03u)); wait_vbl_done(); delay(250u); }
}
void main(void) {
  ...
  while (1) {
    ...
    if (!(previous_keys & J_A) && (keys & J_A) && active_screen == SCREEN_DICE && dice_count != DICE_TARGET) { play_confirm_sound(); dice_rolls[dice_count] = (uint8_t)('0' + dice_value); dice_audit = (uint16_t)(((dice_audit << 5) | (dice_audit >> 11)) ^ dice_value); dice_count++; if (dice_count == DICE_TARGET) { sha256_hash(dice_rolls, DICE_TARGET, dice_entropy); make_mnemonic_words(); seed_session_loaded = 1u; draw_active_screen(); } else update_dice_progress(); }
