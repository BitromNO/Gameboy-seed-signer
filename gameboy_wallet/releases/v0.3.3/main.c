...
static void draw_coin_frame(uint8_t frame) {
  uint8_t row;
  uint16_t bitmap;
  ...
  set_bkg_tiles(9, 7, 2, 2, coin_tiles);
}
...
static void show_boot_logo(void) {
  ...
  for (tick = 0; tick != 20u; tick++) { draw_coin_frame((uint8_t)((tick / 5u) & 0x03u)); wait_vbl_done(); delay(250u); }
}
void main(void) {
