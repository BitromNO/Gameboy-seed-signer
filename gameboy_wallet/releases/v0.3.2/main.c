...
static uint8_t dice_value = 1u, dice_count, dice_page;
...
static void draw_active_screen(void) {
  ...
  if (active_screen == SCREEN_READY) {
    ...
    if (dice_count >= DICE_TARGET) {
      ...
      print_centered(14, "A:NEXT PAGE"); print_centered(16, "B:WIPE AND EXIT");
    } else {
...
void main(void) {
  ...
  while (1) {
    ...
    if (!(previous_keys & J_A) && (keys & J_A) && active_screen == SCREEN_DICE && dice_count == DICE_TARGET) { play_confirm_sound(); dice_page = dice_page == 3u ? 0u : dice_page + 1u; draw_active_screen(); }
    if (!(previous_keys & J_B) && (keys & J_B) && active_screen == SCREEN_DICE) { wipe_dice_material(); active_screen = SCREEN_MENU; draw_menu(); }
